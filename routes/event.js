...
router.get("/search-event", (req, res, next) => {
  ...
  axios
    .get(
      `https://app.ticketmaster.com/discovery/v2/events?&size=30&keyword=${searchEvent}&apikey=${process.env.CONSUMER_KEY}`
    )
    .then((response) => {
...
router.get("/", (req, res, next) => {
  axios
    .get(
      `https://app.ticketmaster.com/discovery/v2/events?&size=12&sort=random&apikey=${process.env.CONSUMER_KEY}`
    )
    .then((response) => {
      // console.log(response);
      const eventArray = response.data["_embedded"].events;
...
router.get("/search-attractions", (req, res, next) => {
  ...
  axios
    .get(
      `https://app.ticketmaster.com/discovery/v2/attractions?&size=30&keyword=${searchEvent}&apikey=${process.env.CONSUMER_KEY}`
    )
    .then((response) => {
      // console.log(response);
...
router.get("/attractions", (req, res, next) => {
  axios
    .get(
      `https://app.ticketmaster.com/discovery/v2/attractions?&size=12&sort=random&apikey=${process.env.CONSUMER_KEY}`
    )
    .then((response) => {
      // console.log(response);
      const attractionArray = response.data["_embedded"].attractions;
